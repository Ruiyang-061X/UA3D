...
class PointNet2MSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        for k in range(self.model_cfg.FP_MLPS2.__len__()):
            pre_channel = self.model_cfg.FP_MLPS2[k + 1][-1] if k + 1 < len(self.model_cfg.FP_MLPS2) else channel_out
            self.FP_modules2.append(
                pointnet2_modules.PointnetFPModule(
                    mlp=[pre_channel + skip_channel_list[k]] + self.model_cfg.FP_MLPS2[k]
                )
            )
        
        self.num_point_features2 = self.model_cfg.FP_MLPS2[0][-1]
    ...
    def forward(self, batch_dict):
        ...
        l_xyz2, l_features2 = l_xyz.copy(), l_features.copy()
        ...
        for i in range(-1, -(len(self.FP_modules2) + 1), -1):
            l_features2[i - 1] = self.FP_modules2[i](
                l_xyz2[i - 1], l_xyz2[i], l_features2[i - 1], l_features2[i]
            )  # (B, C, N)

        point_features2 = l_features2[0].permute(0, 2, 1).contiguous()  # (B, N, C)
        batch_dict2['point_features'] = point_features2.view(-1, point_features2.shape[-1])
        batch_dict2['point_coords'] = torch.cat((batch_idx[:, None].float(), l_xyz2[0].view(-1, 3)), dim=1)

        return batch_dict, batch_dict2
