...
class PointHeadTemplate(nn.Module):
    ...
    def get_box_layer_loss(self, tb_dict=None, forward_ret_dict_full=None):
        pos_mask = self.forward_ret_dict['point_cls_labels'] > 0
        ...
        uncertainty = torch.abs(forward_ret_dict_full['1.pointhead']['point_box_preds'] -
                                forward_ret_dict_full['2.pointhead']['point_box_preds'])
        ...
        print('uncertainty mean: %.4f' % torch.mean(uncertainty))
        print('uncertainty max: %.4f' % torch.max(uncertainty))
        print('uncertainty min: %.4f' % torch.min(uncertainty))

        point_loss_box_src = point_loss_box_src * torch.exp(-uncertainty) + 0.00001 * uncertainty
